...
import { fetchAllDashboardData, type DashboardData as ClientDashboardData } from '@/lib/userDashboard';
import type { DashboardData, RecentActivity } from '@/lib/userDashboard.server';
import type { BookRecommendation, Book, Borrow, Profile } from '@/types';
...
export default function DashboardClient({ profile, userId, initialData }: DashboardClientProps) {
  // Dashboard data — initialized with server-fetched data (no loading skeleton needed)
  const [data, setData] = useState<ClientDashboardData>({
    dueSoon: initialData.dueSoon,
    overdue: initialData.overdue,
    currentlyBorrowed: initialData.currentlyBorrowed,
    friendsBorrows: initialData.friendsBorrows,
  });
  ...
  // Refresh dashboard data client-side (after mutations like returning a book)
  const refreshDashboard = useCallback(async () => {
    try {
      const dashboardData = await fetchAllDashboardData(userId);
  ...
  // Recommendations (separate fetch — still client-side because it may call the AI API)
  const fetchRecommendations = useCallback(async (forceRefresh = false) => {
    setRecommendationsLoading(true);
    setRecommendationsError(null);
    setShowingStaleRecommendations(false);
    try {
      const result = await getUserRecommendations(userId, { forceRefresh });
  ...
  const handleMoreLikeThis = async (rec: BookRecommendation) => {
    try {
      await logActivity(userId, 'recommendation_more_like_this', {
        title: rec.title,
        author: rec.author,
        genre: rec.genre,
      });
  ...
  const handleDismissRec = async (rec: BookRecommendation) => {
    ...
    try {
      await logActivity(userId, 'recommendation_dismissed', {
        title: rec.title,
        author: rec.author,
      });
    } catch {
